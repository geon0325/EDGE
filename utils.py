...
def get_item_attr(rating_mat, num_classes=5):
    ...
    head = torch.nonzero(pop_grp == (num_classes - 1)).view(-1)
    
    return popularity, pop_grp, num_classes, num_grp, head

def get_user_attr(rating_mat, num_classes=5):
    """
    Get item popularity
    """
    popularity = get_item_popularity(rating_mat.T)
    val, indices = torch.topk(popularity, len(popularity))
    pop_grp = torch.zeros(popularity.shape)
    num_grp = np.zeros(num_classes)
    num_grp_item = len(popularity) // num_classes
