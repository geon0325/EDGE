...
class LightGCN(nn.Module):
    def __init__(self, n_u, n_i, d, train_csr, adj_norm, l, lambda_1, dropout, batch_user, device, args, popularity):
        super(LightGCN,self).__init__()
        self.device = device
        self.args = args
        self.degree = popularity
    ...
    def forward(self, uids, pos, neg, head=None, csr=None, test=False):
        if test==True:  # testing phase
            E_u_emb = sum(self.E_u_list)[uids]
            E_i_emb = sum(self.E_i_list)
            
            preds = E_u_emb @ E_i_emb.T
